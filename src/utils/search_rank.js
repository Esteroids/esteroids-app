...
export const stemAndCleanTerm = (term) => {
  return term.replaceAll(/\s\s*/g, ' ').trim().toLowerCase()
}

export const getMultiWords = (searchTerm) => {
...
export const getSearchRank = (site, searchTerm, multiWords) => {
  let rank = 0
  ...
  for (var i in MATCH_BY) {
    ...
    if (cleaned_str.indexOf(searchTerm) !== -1) {
      ...
      if (multiWords !== null) {
        ...
        let matched_all_words = multiWords.every(function (element) {
          return element !== '' && cleaned_str.indexOf(element) !== -1
        })

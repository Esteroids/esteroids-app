  ...
  const connectWallet = useCallback(async () => {
    try {
      ...
      if (network.chainId == 1) {
        ...
        if (accounts) {
          newWeb3Info.account = accounts[0]
          newWeb3Info.isConnected = true
        }

        setWeb3Info(newWeb3Info)
      } else {
        setWeb3Info({ ...web3Info, isConnect: false, connectError: 'Wrong Chain, You  connect to Mainnet' })
        console.error('ChainId:', network.chainId, 'Wrong Chain, You  connect to Mainnet')
        web3Modal.clearCachedProvider()
      }
    } catch (error) {
      setWeb3Info({ ...web3Info, isConnect: false, connectError: null })
      web3Modal.clearCachedProvider()
    }
  }, [web3Info])

  const refreshState = (web3InfoNow) => {
    setWeb3Info({ ...web3InfoNow, account: '', network: null, isConnected: false, chainId: 1, connectError: null })
  }

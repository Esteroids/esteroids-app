...
const BROWSE_PATHS = ["/", "/popular", "/new", "/recent"]
...
const MainApp = () => {
    ...
    const defaultGatway = useMemo(() => EnsSite.getGateway(window?.location?.href), []);

    useEffect(() => {  
        const eType = 'page'
        const meta = {loc: (location?.pathname || '') + (location?.search || ''), host: window?.location?.host}
        add({eType, meta});

...
function Cards(props) {
  ...
  for (let i=0; i<props.currentResultsShown; i++) {
    cards.push(<SiteCard key={i} location="search_results" site = {dwebData['sites'][props.searchResults[i]]} defaultGatway={props.defaultGatway} />);
  }
...
function SearchResults({defaultGatway}){
    ...
    useEffect(() => {
      ...
      if(searchTerm) {
        const eType = 'search'
        const meta = {searchTerm, host: window?.location?.host}
        add({eType, meta});
      }
    }, [searchTerm, add])
    ...
    return (
      <div className="container">
        ...
        <Cards searchTerm = {searchTerm} searchResults={search_results} currentResultsShown={currentResultsShown} defaultGatway={defaultGatway} />

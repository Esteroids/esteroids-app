...
const SiteCard = props => {
    ...
    const goToPage = async (event) =>{ 
        ...
        const eType = 'link'
        const meta = {'site': siteAddress, host: window?.location?.host, from: (pageLocation?.pathname || '') + (pageLocation?.search || ''), d_surf: isConnected}
    ...
    return (
        <a href={getLink(site, defaultGatway)} onClick={goToPage} target="_blank" rel="noreferrer" className={getColumnSize(location) + " full-site-card"}>
            <div className={getColumnSize(location)}>
                <div className="card-esteroids">
                    <div className={"card-upper-rect py-2 " + isFirst(first) + " d-flex flex-column justify-content-center align-items-center"}>
                        <LazyLoad height={100} placeholder={(<PlaceholderComponent className="card-img" width={101} height={101} />)} once>
